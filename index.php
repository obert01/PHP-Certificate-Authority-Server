...
switch ($menuoption) {
  ...
  case "sign_csr":
    ...
    sign_csr($config['capassword'],$page_variables['csr_name'],$page_variables['days'],$page_variables['device_type'], $page_variables['subjectAltName']);
    printFooter();
    break;
  ...
  case "renew_ca_form":
    printHeader('Renew the current CA - Part 1');
    renew_ca_form();
    printFooter();
    break;

  case "renew_ca":
    printHeader('Renew the Current CA - Part 2');
    renew_ca($page_variables['days']);  
    printFooter();
    break;
