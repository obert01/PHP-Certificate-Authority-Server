...
function create_cert_store($my_certstore_path, $my_common_name) {
  ...
  $orig_fp = fopen('./include/renewca.conf',"r") or die('Unable to open OPENSSL.CONF template');
  $new_fp = fopen($config['ca_path']."renewca.conf","w") or die('Unable to open OPENSSL.CONF new file');
  while (!feof($orig_fp)) {
    $this_line = rtrim(fgets($orig_fp));
    fwrite($new_fp,$this_line."\n");
  }
