...
?>
 ...
 while (($file = readdir($dh)) !== false) {
   if ( ($file !== ".htaccess") && ($query==="" || stripos($file, $query)!==FALSE) && is_file($config['req_path'].$file) )  {
     $filename = substr($file, 0,strrpos($file,'.'));
     $ext = substr($file, strrpos($file,'.'));
     if (preg_match("((.*) - (.*) - (.*))",$filename,$matches)==1)
       $this_dn=array("cn"=>trim($matches[2]),"o"=>trim($matches[1]),"emailAddress"=>trim($matches[3]));
     else if (preg_match("((.*) - (.*))",$filename,$matches)==1)
       $this_dn=array("cn"=>trim($matches[2]),"o"=>trim($matches[1]));
 ...
 while (($file = readdir($dh)) !== false) {
   if ( ($file !== ".htaccess") && ($query==="" || stripos($file, $query)!==FALSE) && is_file($config['cert_path'].$file) )  {
     $filename = substr($file, 0,strrpos($file,'.'));
     $ext = substr($file, strrpos($file,'.'));
     if (preg_match("((.*) - (.*) - (.*))",$filename,$matches)==1)
       $this_dn=array("cn"=>trim($matches[2]),"o"=>trim($matches[1]),"emailAddress"=>trim($matches[3]));
 ...
 while (($file = readdir($dh)) !== false) {
   if ( ($file !== ".htaccess") && ($query==="" || stripos($file, $query)!==FALSE) && is_file($config['key_path'].$file) )  {
     $filename = substr($file, 0,strrpos($file,'.'));
     $ext = substr($file, strrpos($file,'.'));
     if (preg_match("((.*) - (.*) - (.*))",$filename,$matches)==1)
       $this_dn=array("cn"=>trim($matches[2]),"o"=>trim($matches[1]),"emailAddress"=>trim($matches[3]));
 ...
 while (!feof($my_index_handle)) {
   ...
   if (preg_match($pattern,$this_line,$matches)) {
     if ($query === NULL || ($query !== "" && stripos($matches[6], $query) === FALSE))
       continue;
     ...
     $this_pattern2 = "(/C(.*)/O=(.*)/OU=(.*)/CN=([^/]*)(/emailAddress=(.*))?)";
     ...
     if (preg_match($this_pattern2,$my_index_name,$matches)==1) {
       ...
       $this_dn['cn']=$matches[4];
       $this_dn['c']=$matches[1];
       $this_dn['o']=$matches[2];
       $this_dn['ou']=$matches[3];
       $this_dn['emailAddress']=$matches[6];
