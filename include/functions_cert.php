	...
	<tr><td width=100>Name:<td><select name="cert_name" rows="6">
	  <option value="">--- Select a certificate
	    ...
	    while (($file = readdir($dh)) !== false) {
	      if ( ($file !== ".htaccess") && is_file($config['cert_path'].$file) && strpos($config['cert_path'].$file, ".p12") )  {
		$name = substr($file, 0,strrpos($file,'.'));
		$ext = substr($file, strrpos($file,'.'));
		$certs[$name.$ext] = $name.$ext;
	      }
	    }
	    asort($certs);
...
function download_cert($this_cert,$cer_ext) {
  ...
  else
  {
    ...
    $download_certfile = $config['cert_path']. $filename.$ext;
    $application_type='application/octet-stream';
  }
...
?>
  <p>
	...
	<table  style="width: 400px;">
	  <tr><td width=100>Passphrase:<td><input type="text" name="pkey_pass" value="<?php print htmlspecialchars(generatePassword());?>"/>
	    <tr><td width=100>Name:<td><select name="cert_name" rows="6">
	      <option value="">--- Select a certificate
		<?php
		$dh = opendir($config['cert_path']) or die('Unable to open cert path');
...
function convert_cert_pkcs12($this_cert_name,$my_pkey_pass,$my_pkcs12_pass){
  $config=$_SESSION['config'];
  $name = substr($this_cert_name, 0,strrpos($this_cert_name,'.'));
  $ext = substr($this_cert_name, strrpos($this_cert_name,'.'));
  $my_certfile=$name.$ext;
  ...
  $fp = fopen($config['cert_path'].$my_certfile, "r") or die('Fatal: Error opening Certificate');
  $my_cert = fread($fp, filesize($config['cert_path'].$my_certfile)) or die('Fatal: Error reading Certificate');
  $my_cert_parse = openssl_x509_parse($my_cert);
...
?>
  <form action="index.php" method="post">
    <input type="hidden" name="menuoption" value="download_cert">
    <input type="hidden" name="cert_name" value="<?PHP print $name.'.p12';?>">
  ...
  <p>
	...
	<tr><td width=100>Name:<td><select name="cert_serial">
	  <option value="">--- Select a certificate
		...
		if ($matches[1] == 'V') {
		  $dn = explode("/", $matches[6]);
		  $cn = substr($dn[4], strpos($dn[4],"=") + 1);
		  if (!empty($dn[5]))
		    $email = substr($dn[5], strpos($dn[5],"=") + 1);
		  $o = substr($dn[2], strpos($dn[2],"=") + 1);
		  if (!empty($email))
		    $certs[$matches[4]] = $o . " - " . $cn . " - " . $email;
		  else
		    $certs[$matches[4]] = $o . " - " . $cn;
		}
	      }
	    }
	    ...
	    asort($certs);
	    while (list($key,$val) = each($certs)) {
	      if ( $my_values['cert_serial'] == $key) $this_selected=" selected=\"selected\""; else $this_selected="";
	      print "<option value=\"".$key."\" $this_selected>".$val."</option>\n";
	    }
	...
	<table  style="width: 400px;">
	  <tr><td>Name:<td><select name="cert_name" rows="6">
	    <option value="">--- Select a Certificate
	      ...
	      $certs = array();
	      while (($file = readdir($dh)) !== false) {
		if ( ($file !== ".htaccess") && is_file($config['cert_path'].$file) && strpos($config['cert_path'].$file, ".pem") )  {
		  $name = substr($file, 0,strrpos($file,'.'));
		  $ext = substr($file, strrpos($file,'.'));
		  $certs[$name.$ext] = $name.$ext;
		}
...
function view_cert($my_certfile) {
  ...
  {
    $name = substr($my_certfile, 0,strrpos($my_certfile,'.'));
    $ext = substr($my_certfile, strrpos($my_certfile,'.'));
    $my_certfile=$name.$ext;
    $my_cert = openssl_x509_parse(file_get_contents($config['cert_path'].$my_certfile));
  }
