...
?>
  <p>
	...
	<tr><th width=100>Common Name (eg root-ca.golf.local)</th><td><input type="text" name="cert_dn[commonName]" value="" size="40"></td></tr>
	<tr><th>Contact Email Address</th><td><input type="text" name="cert_dn[emailAddress]" value="" size="30"></td></tr>
	...
	<tr><th>City</th><td><input type="text" name="cert_dn[localityName]" value="" size="25"></td></tr>
	<tr><th>State or province name</th><td><input type="text" name="cert_dn[stateOrProvinceName]" value="" size="25"></td></tr></td></tr>
...
?>
  <p>
    <b>Renew a Certificate Authority</b><br/>
    <form action="index.php" method="post">
      <input type="hidden" name="renew_ca" value="renew_ca"/>
      <input type="hidden" name="menuoption" value="renew_ca"/>
      <input type="hidden" name="device_type" value="ca_cert"/>
      <table  style="width: 400px;">
	<tr><th>Number of Days</th><td><input type="text" name="days" size="4" value="7300" /></td></tr>
	<tr><td><td><input type="submit" value="Renew CA"/>
      </table>
    </form> 
  </p>
...
function renew_ca($my_days) {
  $config = $_SESSION['config'];
  $output = array();
  exec("/usr/bin/openssl x509 -certopt no_header -x509toreq -in " . escapeshellarg($config['cacert'])
     . " -signkey " . escapeshellarg($config['cakey'])
     . " -passin pass:" . escapeshellarg($config['capassword'])
     . " -out " . escapeshellarg($config['req_path'] . "cacert.csr") . " 2>1", $output, $error);
  if ($error)
    die("Can' obtain CSR from CA - " . $output);
  sign_csr($config['capassword'],"cacert.csr",$my_days,"ca_cert");
}
